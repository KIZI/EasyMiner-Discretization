...
import eu.easyminer.discretization.interval.{ExclusiveIntervalBound, InclusiveIntervalBound, Interval}
...
class EquidistantIntervals[T](bins: Int)(implicit val n: Numeric[T]) extends Discretization[T] {
  ...
  def discretize(data: Iterable[T]): Seq[Interval] = data.iterator
    .map(x => (x, x))
